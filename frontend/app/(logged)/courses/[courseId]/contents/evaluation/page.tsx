...
export default function CreateEvaluationPage({ params }: Params) {
  ...
  const isValid = !titleError && !fileError && !contentError && !dueDateError;
  ...
  const normalizeDueDate = (value: string) => {
    // Ensure "YYYY-MM-DDTHH:mm:ss" for better backend parsing
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const resp = await contentController.createEvaluation(params.courseId, form, accessToken);
      if (resp.success) {
        router.push(`/courses/${params.courseId}?created=1`);
      } else {
        setError(resp.message || 'No se pudo crear la evaluación');
      }
    } catch (err) {
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-secondary-color-70">Nueva evaluación</h1>
      ...
      <form onSubmit={onSubmit} className="space-y-6">
        ...
        <div className="space-y-1">
          <label className="block text-sm font-medium text-gray-700">Fecha límite de entrega</label>
          <input
            type="datetime-local"
            value={dueDate}
